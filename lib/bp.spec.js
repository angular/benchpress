...
describe('bp', function() {
  ...
  beforeEach(function() {
    ...
    bp.Document._container.appendChild(scriptTemplate);
    bp.Runner.runState = {
      iterations: 0,
      numSamples: 20,
      recentResult: {}
  ...
  describe('.Document', function() {
    ...
    describe('.loadNextScript()', function() {
      ...
      it('should call addScriptToUI with config with correct src', function() {
        var spy = spyOn(bp.Document, 'addScriptToUI');
        bp.Document.loadNextScript();
        expect(spy.calls[0].args[0]).toEqual({id: 'angular', src: 'foobar'});
      })
    });
    ...
    xdescribe('.addScriptToUI()', function() {
      beforeEach(function() {
        bp.Document._scriptsContainer = document.createElement('div');
        bp.Document._scriptsContainer.classList.add('scripts');
      ...
      it('should add a script to the info container', function() {
        var appendSpy = spyOn(bp.Document._scriptsContainer, 'appendChild');
        bp.Document.addScriptToUI({src: '/foo.js', id: 'foo'});
    ...
    xdescribe('.addScriptsContainer()', function() {
      it('should set the container to bp.Document._scriptsContainer');
      it('should add script template to bp.Document');
    });
