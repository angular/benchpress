  ...
  function buildIt (err) {
    ...
    fs.readFile(path.resolve(libPath, 'template.html'), function(err, template) {
      ...
      benchmarks.forEach(function(benchmark) {
        ...
        browserify(path.resolve(benchpressBase , './lib/bp'), {baseDir: './lib/'}).bundle().
            pipe(fs.createWriteStream(path.resolve(buildPath, benchmark, 'bp.js'))).
            on('close', isDone);
        browserify(path.resolve(benchpressBase, './lib/auto-bp'), {baseDir: './lib/'}).bundle().
            pipe(fs.createWriteStream(path.resolve(buildPath, benchmark, 'auto-bp.js'))).
            on('close', isDone);
