...
exports.launchChrome = function launchChrome() {
  if (process.platform == 'linux') {
    require('child_process').exec('google-chrome --js-flags="--expose-gc"', function(err) {
      if (err) console.error('An error occurred trying to launch Chrome: ', err);
    });
  }
  else if (process.platform === 'darwin') {
    require('child_process').exec('/Applications/Google\\ Chrome\\ Canary.app/Contents/MacOS/Google\\ Chrome\\ Canary --enable-memory-info --enable-precise-memory-info --enable-memory-benchmarking --js-flags="--expose-gc"', function(err) {
      if (err) console.error('An error occurred trying to launch Chrome: ', err);
    });
  }
  else {
