...
function Runner (doc, globals, measure, report, runState, steps, values) {
  this._doc = doc;
...
Runner.prototype.runAllTests = function (done) {
  ...
  if (this._runState.iterations--) {
    ...
    this._report.markup = this._report.calcStats();
    this._doc.writeReport(this._report.markup);
...
di.annotate(Runner, new di.Inject(Document, Globals, Measure, Report, RunState, Steps, Values));
